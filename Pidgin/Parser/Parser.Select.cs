...
namespace Pidgin
{
    public abstract partial class Parser<TContext, TToken, T>
    {
        ...
        /// This is an infix synonym for <see cref="Parser.Map{TContext, TToken, T1, R}(Func{T1, R}, Parser{TContext, TToken, T1})"/>.
        /// </summary>
        ...
        public Parser<TContext, TToken, U> Select<U>(Func<T, U> selector)
        {
            if (selector == null)
            {
        ...
        /// <param name="selector">A transformation function.</param>
        /// <returns>A parser which applies <paramref name="selector"/> to the result of the current parser.</returns>
        public Parser<TContext, TToken, U> Map<U>(Func<T, U> selector)
        {
            if (selector == null)
            {
                throw new ArgumentNullException(nameof(selector));
