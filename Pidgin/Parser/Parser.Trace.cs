...
namespace Pidgin
{
    public partial class Parser<TContext, TToken, T>
    {
        ...
        public Parser<TContext, TToken, T> Trace(Func<T, string> message)
        {
            if (message == null)
            {
        ...
        /// <param name="message">A message to write to the console.</param>
        /// <returns>A parser which runs the current parser and prints the given message to the console.</returns>
        public Parser<TContext, TToken, T> Trace(string message)
        {
            if (message == null)
            {
                throw new ArgumentNullException(nameof(message));
        ...
        public Parser<TContext, TToken, T> TraceResult() => Trace(x => x!.ToString()!);
    }
}
