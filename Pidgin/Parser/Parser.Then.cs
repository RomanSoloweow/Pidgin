...
namespace Pidgin
{
    public abstract partial class Parser<TContext, TToken, T>
    {
        ...
        public Parser<TContext, TToken, U> Then<U>(Parser<TContext, TToken, U> parser)
        {
            if (parser == null)
            {
        ...
        /// <returns>A parser which applies the current parser followed by <paramref name="parser"/>.</returns>
        public Parser<TContext, TToken, R> Then<U, R>(Parser<TContext, TToken, U> parser, Func<T, U, R> result)
        {
            if (parser == null)
            {
                throw new ArgumentNullException(nameof(parser));
        ...
        public Parser<TContext, TToken, U> Then<U>(Func<T, Parser<TContext, TToken, U>> selector)
        {
            if (selector == null)
            {
        ...
        /// <returns>A parser which applies the current parser before applying the result of the <paramref name="selector"/> function.</returns>
        public Parser<TContext, TToken, R> Then<U, R>(Func<T, Parser<TContext, TToken, U>> selector, Func<T, U, R> result)
        {
            if (selector == null)
            {
                throw new ArgumentNullException(nameof(selector));
