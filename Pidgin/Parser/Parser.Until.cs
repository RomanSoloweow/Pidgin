...
namespace Pidgin
{
    public abstract partial class Parser<TContext, TToken, T>
    {
        ...
        public Parser<TContext, TToken, IEnumerable<T>> Until<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
        ...
        /// <param name="terminator">A parser to parse a terminator.</param>
        /// <returns>A parser which applies this parser repeatedly until <paramref name="terminator"/> succeeds.</returns>
        public Parser<TContext, TToken, (IEnumerable<T>, U)> ManyThen<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
        ...
        /// until <paramref name="terminator"/> succeeds.
        /// </returns>
        public Parser<TContext, TToken, IEnumerable<T>> AtLeastOnceUntil<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
        ...
        /// until <paramref name="terminator"/> succeeds.
        /// </returns>
        public Parser<TContext, TToken, (IEnumerable<T>, U)> AtLeastOnceThen<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
            ...
            return new AtLeastOnceThenParser<TContext, TToken, T, U>(this, terminator, true)!;
        }
        ...
        public Parser<TContext, TToken, Unit> SkipUntil<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
        ...
        /// </returns>
        public Parser<TContext, TToken, U> SkipManyThen<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
        ...
        /// <paramref name="terminator"/> succeeds, discarding the results.
        /// </returns>
        public Parser<TContext, TToken, Unit> SkipAtLeastOnceUntil<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
        ...
        /// <paramref name="terminator"/> succeeds, discarding the results.
        /// </returns>
        public Parser<TContext, TToken, U> SkipAtLeastOnceThen<U>(Parser<TContext, TToken, U> terminator)
        {
            if (terminator == null)
            {
                throw new ArgumentNullException(nameof(terminator));
            ...
            return new AtLeastOnceThenParser<TContext, TToken, T, U>(this, terminator, false).Select(tup => tup.Item2);
        }
    }
    ...
    internal sealed class AtLeastOnceThenParser<TContext, TToken, T, U> : Parser<TContext, TToken, (IEnumerable<T>?, U)>
        where TContext : IParsingContext
    {
        private readonly Parser<TContext, TToken, T> _parser;
        private readonly Parser<TContext, TToken, U> _terminator;
        private readonly bool _keepResults;

        public AtLeastOnceThenParser(Parser<TContext, TToken, T> parser, Parser<TContext, TToken, U> terminator, bool keepResults)
            : base()
        {
            _parser = parser;
        ...
        public sealed override bool TryParse(ref TContext context, ref ParseState<TToken> state, ref PooledList<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out (IEnumerable<T>?, U) result)
        {
            ...
            while (true)
            {
                ...
                var terminatorSuccess = _terminator.TryParse(ref context, ref state, ref terminatorExpecteds, out var terminatorResult);
                ...
                var itemSuccess = _parser.TryParse(ref context, ref state, ref itemExpecteds, out var itemResult);
