namespace Pidgin
{
    public partial class Parser<TContext, TToken, T>
    {
        ...
        public Parser<TContext, TToken, U> WithResult<U>(U result)
            => Select(_ => result);
        ...
        /// <typeparam name="U">The type of the result.</typeparam>
        /// <returns>A parser which behaves like the current parser but returns <paramref name="result"/>.</returns>
        public Parser<TContext, TToken, U> ThenReturn<U>(U result)
            => Select(_ => result);
        ...
        public Parser<TContext, TToken, Unit> IgnoreResult()
