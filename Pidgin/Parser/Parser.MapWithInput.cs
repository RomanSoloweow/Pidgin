...
namespace Pidgin
{
    public abstract partial class Parser<TContext, TToken, T>
    {
        ...
        public Parser<TContext, TToken, U> MapWithInput<U>(ReadOnlySpanFunc<TToken, T, U> selector)
        {
            if (selector == null)
            {
            ...
            return new MapWithInputParser<TContext, TToken, T, U>(this, selector);
        }
    }
    ...
    internal class MapWithInputParser<TContext, TToken, T, U> : Parser<TContext, TToken, U>
        where TContext : IParsingContext
    {
        ...
        public MapWithInputParser(Parser<TContext, TToken, T> parser, ReadOnlySpanFunc<TToken, T, U> selector)
        {
            _parser = parser;
            _selector = selector;
        }
